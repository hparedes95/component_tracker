...
async function tryDiscover(url, parser) {
  ...
  try { const html = await loadRendered(url); return html ? parser(html) : null; }
  catch { return null; }
}
  ...
  for (const domain of domains.slice(0, 20)) {
    ...
    if (domain.startsWith('amazon')) {
      ...
      if (!url) url = await tryDiscover(bingSearchUrl(query, domain), (h) => {
        const hits = parseBingResults(h, domain); return hits.length ? hits[0] : null;
      });
    }
...
ipcMain.handle('scan', async (_e, url) => {
  ...
  try { const html = await loadRendered(url); return html ? extractProducts(html) : []; }
  catch { return []; }
});
...
app.whenReady().then(() => {
  ...
  setInterval(() => {
    if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send('auto-refresh');
  }, REFRESH_INTERVAL_MS);
  app.on('activate', () => {
